...
export function AccordionSection({ title, children, defaultOpen = true, className, badge }: { title: ReactNode; children: ReactNode; defaultOpen?: boolean; className?: string; badge?: ReactNode }) {
  ...
  return (
    <div className={cn('rounded-xl border border-zinc-200 bg-white', className)}>
      <button type="button" onClick={() => setOpen((o) => !o)} aria-expanded={open} aria-controls={panelId} className="flex w-full items-center justify-between gap-3 px-4 py-3 text-left focus-ring rounded-xl">
      ...
      {open && <div id={panelId} className="space-y-4 border-t border-zinc-100 px-4 py-4">{children}</div>}
    </div>
  );
}
